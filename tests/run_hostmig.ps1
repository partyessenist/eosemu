# Builds tests/hostmig.c and runs two instances (host + join): the host leaves
# its own lobby and the joiner must be promoted to owner (host migration).
...
$outDir = Join-Path $env:TEMP "eosemu_hostmig"
...
$src = Join-Path $root "tests\hostmig.c"
$exe = Join-Path $outDir "hostmig.exe"
$obj = Join-Path $outDir "hostmig.obj"
...
$joinProc.WaitForExit(60000) | Out-Null
...
$joinPass = (Select-String -Path $joinOut -Pattern "HOSTMIG-JOIN PASS" -Quiet)
$hostPass = (Select-String -Path $hostOut -Pattern "HOSTMIG-HOST PASS" -Quiet)
if ($joinPass -and $hostPass) { Write-Host "HOST MIGRATION: PASS"; exit 0 }
Write-Host "HOST MIGRATION: FAIL (host=$hostPass join=$joinPass)"; exit 1
