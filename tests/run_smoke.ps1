# Builds and runs the EOSEmu smoke test.
# Compiles tests/smoke.c against the SDK import library, drops the freshly built
# EOSEmu DLL beside the exe (so it resolves by name), and runs it.
...
$outDir = Join-Path $env:TEMP "eosemu_smoke"
...
$src = Join-Path $root "tests\smoke.c"
$exe = Join-Path $outDir "smoke.exe"
$obj = Join-Path $outDir "smoke.obj"
$installerDir = Split-Path $vswhere -Parent

# vcvars64.bat shells out to vswhere by bare name, so put the installer dir on
# PATH for the child cmd. Build and run in one batch; check the exit code
# rather than letting a native stderr line abort the script.
