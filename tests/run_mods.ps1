# Builds and runs the EOSEmu mods test.
# Writes an eosemu.ini declaring two mods, points %EOSEMU_CONFIG% at it, and
# runs mods.exe against the freshly built EOSEmu DLL. The expected values here
# must match tests/mods.c.
...
$src = Join-Path $root "tests\mods.c"
$exe = Join-Path $outDir "mods.exe"
$obj = Join-Path $outDir "mods.obj"
...
Write-Host "--- running mods test (EOSEMU_CONFIG=$iniPath) ---"
& $exe
$code = $LASTEXITCODE
