# Builds and runs the EOSEmu reports test.
# Compiles tests/reports.c against the SDK import library, drops the freshly
# built EOSEmu DLL beside the exe, and runs it. Asserts a valid player report
# completes Success and an invalid one completes InvalidParameters.
...
$outDir = Join-Path $env:TEMP "eosemu_reports"
...
$src = Join-Path $root "tests\reports.c"
$exe = Join-Path $outDir "reports.exe"
$obj = Join-Path $outDir "reports.obj"
...
Write-Host "--- running reports test ---"
& $exe
$code = $LASTEXITCODE
